...
from dissector.plugins.libs.androguard.androguard.core.androconf import error, warning
from dissector.plugins.libs.androguard.androguard.core.bytecodes import jvm, dvm
from dissector.plugins.libs.androguard.androguard.core.bytecodes.api_permissions import DVM_PERMISSIONS_BY_PERMISSION, DVM_PERMISSIONS_BY_ELEMENT
      ...
      try :
            ...
            if (op_value >= 0x52 and op_value <= 0x6d) :
                ...
                if self.tainted_packages != None :
                    # if "Landroid/content/Intent;-><init>" in i.get_output():
                    #     print i.get_name() + " " + i.get_output()
                    #     print method_info[0]
                    #     print "Adding to packagees"
...
class TaintedPackages :
    ...
    def _add_pkg(self, name) :
        if name not in self.__packages :
            if "Landroid/content/Intent" in name:
    ...
    def push_info(self, class_name, access, idx, method, idx_method) :
        ...
        try :
            ...
            # if "Landroid/content/Intent" in class_name:
            #     print "Added."
            #     print show_Path(self.__vm, p)
        except :
            try :
            ...
            for j in paths:
                if j.get_access_flag() == TAINTED_PACKAGE_CALL:
                    ...
                    if dst_class_name in classes and m.get_name() in classes or interesting:
                        l.append(j)
                        if "Landroid/content/Intent;<init>" in "%s%s%s" % j.get_dst(self.__vm.get_class_manager()):
                            print "Appeso!"
                            print show_Path(self.__vm, j)
        return l
...
from dissector.plugins.libs.androguard.androguard.core import analysis
